...
import { isExtensionRulesEnabled } from '../utils/isExtensionRulesEnabled';
...
export class ExtensionManager {
  ...
  static resolve(extensions: Extensions): Extensions {
    ...
    if (duplicatedNames.length) {
      console.warn(
        `Duplicate extension names found: [${duplicatedNames
          .map((item) => `'${item}'`)
      ...
      .map((extension) => {
        ...
        if (
          isExtensionRulesEnabled(
            extension,
            Boolean(editor.options.enableInputRules)
          ) &&
          addInputRules
        ) {
        ...
        if (
          isExtensionRulesEnabled(
            extension,
            Boolean(editor.options.enablePasteRules)
          ) &&
          addPasteRules
        ) {
        ...
        >(extension, 'addProseMirrorPlugins', context);
